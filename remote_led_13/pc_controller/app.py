...
API_TOKEN = os.getenv("ARDUINO_API_TOKEN", "")
...
def status(x_api_token: str | None = Header(default=None)) -> CommandResponse:
    require_token(x_api_token)
    return run_command("STATUS")


@app.post("/led/on", response_model=CommandResponse)
def led_on(x_api_token: str | None = Header(default=None)) -> CommandResponse:
    require_token(x_api_token)
    return run_command("LED_ON")


@app.post("/led/off", response_model=CommandResponse)
def led_off(x_api_token: str | None = Header(default=None)) -> CommandResponse:
    require_token(x_api_token)
